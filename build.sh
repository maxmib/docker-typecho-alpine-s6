...
docker build -f ./docker/Dockerfile -t typecho:amd64 .
docker push typecho:amd64
...
docker image prune -f
docker build -f ./docker/Dockerfile.arm64 -t typecho:arm64 .
docker push typecho:arm64
